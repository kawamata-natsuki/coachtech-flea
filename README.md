...
#### テスト目的に応じた設定
・　商品名、ブランド、商品説明は最大文字数を入力し、2行表示・折り返し・はみ出しなどのレイアウト崩れが起きないかを確認
・　すべてのカテゴリを選択状態とし、タグ表示の際の余白や折り返し表示を検証しています
#### 商品画像について
商品画像素材のファイル名は、すべて英数字に変更しています。

---

### 2. 商品カテゴリ情報
`app/Constants/CategoryConstants.php`  にてカテゴリコードと名称を定義し、 `CategorySeeder` でカテゴリマスタを一括登録します。

---

### 3. ユーザー情報
`UserSeeder` により、**一般ユーザー3名と管理者ユーザー1名**が作成されます。
